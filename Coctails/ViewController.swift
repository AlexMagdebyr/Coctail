...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.delegate = self
        let modelsArray = allDrinks.compactMap({CoctailsCellModel(coctailName: $0.strDrink, coctailImage: $0.strDrinkThumb,  labelAlcoholicOrNo: $0.strAlcoholic, indentifier: $0.idDrink)})
        cell.setupWithModel(model: modelsArray[indexPath.row])
        return cell
    }
}

//MARK: - CoctailsCellDelegate
...
//        allDrinks.first(where: { $0.idDrink == identifier})
        let data = allDrinks.first(where: {$0.idDrink == String(index)})
        let controller = ReseptCoctailViewController(strDrink: data?.strDrink, strInstructions: data?.strInstructions)
